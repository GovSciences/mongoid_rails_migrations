namespace :db do
  ...
  task :migrate => :environment do
    ...
    Mongoid::Migrator.migrate(migrations_path, ENV["VERSION"] ? ENV["VERSION"].to_i : nil)
  end
  ...
  namespace :migrate do
    ...
    task :up => :environment do
      ...
      Mongoid::Migrator.run(:up, migrations_path, version)
    end
    ...
    task :down => :environment do
      version = ENV["VERSION"] ? ENV["VERSION"].to_i : nil
      raise "VERSION is required" unless version
      Mongoid::Migrator.run(:down, migrations_path, version)
    end
  end
  ...
  task :rollback => :environment do
    ...
    Mongoid::Migrator.rollback(migrations_path, step)
  end

  def migrations_path
    return Rails.application.config.paths['db/migrate']
  end
